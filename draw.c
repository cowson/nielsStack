...
void add_box( struct matrix * polygons,
  ...
  //top
  add_polygon(polygons, x1, y0, z1, x0, y0, z1, x1, y0, z0);
  add_polygon(polygons, x1, y0, z0, x0, y0, z1, x0, y0, z0);
...
void add_torus( struct matrix * polygons,
  ...
  for (i=0; i < (points->lastcol); i++) {
	...
	add_polygon( polygons,
		     (points->m)[0][i], (points->m)[1][i], (points->m)[2][i],
		     (points->m)[0][n-1], (points->m)[1][n-1], (points->m)[2][n-1],
		     (points->m)[0][i-n+1], (points->m)[1][i-n+1], (points->m)[2][i-n+1]);
	add_polygon( polygons,
